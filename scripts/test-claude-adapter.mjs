...
const processReasoningIndex = events.findIndex((event) => event.rawType === "stream.thinking_delta");
const processAssistantIndex = events.findIndex((event) => event.type === "assistant");
assert.ok(processReasoningIndex >= 0 && processReasoningIndex < processAssistantIndex, "Claude process 必须先发 thinking，再发 assistant 正文");
assert.equal(events.filter((event) => event.type === "reasoning").length, 1, "最终 assistant 消息不能重新发出已流式接收的 thinking");
...
const sessionReasoningIndex = sessionEvents.findIndex((event) => event.rawType === "stream.thinking_delta");
const sessionAssistantIndex = sessionEvents.findIndex((event) => event.type === "assistant");
assert.ok(sessionReasoningIndex >= 0 && sessionReasoningIndex < sessionAssistantIndex, "Claude session 必须先发 thinking，再发 assistant 正文");
assert.equal(sessionEvents.filter((event) => event.type === "reasoning").length, 1, "长驻 Session 不能重新发出已流式接收的 thinking");
