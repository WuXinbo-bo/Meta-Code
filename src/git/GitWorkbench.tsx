...
export function GitWorkbench({ workspaceId }: { workspaceId: string }) {
  ...
  return <section className="git-workbench simplified">
    ...
    <main className="version-visual-layout">
      <section className="development-graph-panel"><header className="panel-heading"><div><strong>发展时间线</strong><span>{filteredCommits.length} 次提交</span></div><label className="version-search"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索发展记录" />{query && <button type="button" aria-label="清除搜索" onClick={() => setQuery("")}><X size={14} /></button>}</label></header><div className="development-graph" role="list">{graphRows.map((row) => <button type="button" role="listitem" className={`development-row ${selectedSha === row.commit.sha ? "selected" : ""}`} key={row.commit.sha} onClick={() => setSelectedSha(row.commit.sha)}><GraphLane row={row} /><span className="development-copy"><strong>{row.commit.subject}</strong><span>{row.commit.author} · {relativeTime(row.commit.authoredAt)}</span>{(row.commit.refs || []).length > 0 && <em>{row.commit.refs.slice(0, 3).join(" · ")}</em>}</span><code>{row.commit.shortSha}</code></button>)}{!graphRows.length && <p className="version-empty">没有匹配的发展记录</p>}</div></section>
