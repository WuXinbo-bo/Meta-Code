...
export function GitWorkbench({ workspaceId }: { workspaceId: string }) {
  ...
  const load = async (background = false) => {
    ...
    setError("");
    try {
      const [nextStatus, nextBranches, nextCommits, nextRemotes, nextActivity] = await Promise.all([
        ...
        request<GitRemote[]>(`/api/workspaces/${encodeURIComponent(workspaceId)}/git/remotes`),
        request<{ records: GitActivity[] }>(`/api/workspaces/${encodeURIComponent(workspaceId)}/git/activity?limit=12`)
      ]);
      setStatus(nextStatus); setBranches(nextBranches); setCommits(nextCommits.commits); setRemotes(nextRemotes); setActivities(nextActivity.records || []);
  ...
  const fetchRemoteRefs = async () => {
    ...
    setError("");
    try {
      await request(`/api/workspaces/${encodeURIComponent(workspaceId)}/git/fetch`, {
        method: "POST",
        body: JSON.stringify({ remote: remotes[0].name }),
        headers: { "Content-Type": "application/json" }
      });
      await load(true);
    } catch (cause) {
  ...
  return <section className="git-workbench">
    <header className="git-toolbar"><div><span className="git-kicker"><GitBranch size={14} />版本管理</span><h1>{status.branch === "(detached)" ? "分离 HEAD" : status.branch}</h1><p>{status.root} · {status.head.slice(0, 8)}</p></div><div className="git-toolbar-actions"><span className={`git-clean-state ${status.conflicted ? "danger" : status.files.length ? "dirty" : "clean"}`}><i />{status.conflicted ? "存在冲突" : status.files.length ? `${status.files.length} 个未提交文件` : "工作树干净"}</span>{remotes.length > 0 && <button type="button" onClick={() => void fetchRemoteRefs()} disabled={fetching || actionBusy} title="仅获取远程引用，不修改当前工作树"><RefreshCw className={fetching ? "spin" : undefined} size={15} />{fetching ? "同步中" : "同步远程"}</button>}<button type="button" onClick={() => void load(true)} disabled={refreshing || fetching} title="刷新版本状态"><RefreshCw className={refreshing ? "spin" : undefined} size={15} />刷新</button></div></header>
    <div className="git-summary"><span><strong>{status.branch}</strong>{status.upstream ? ` · 跟踪 ${status.upstream}` : " · 未设置远程跟踪"}</span><span className="git-sync">↑ {status.ahead} · ↓ {status.behind}</span><span>{remotes.length ? `远程：${remotes.map((remote) => remote.name).join("、")}` : "未配置远程仓库"}</span><span>最近刷新 {relativeTime(status.refreshedAt)}</span></div>
