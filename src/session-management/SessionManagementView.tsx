  ...
  return <section className={`session-management-view ${embedded ? "embedded" : ""}`}>
    ...
    {(section === "inventory" || section === "health") && <div className={`session-console ${focused ? "with-detail" : ""}`}>
      ...
      <main className="session-console-main">
        ...
        <div className="session-management-toolbar"><label><Search size={14} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索会话、路径或线程 ID" />{query && <button aria-label="清除" onClick={() => setQuery("")}><X size={13} /></button>}</label><select value={source} onChange={(event) => setSource(event.target.value)}><option value="all">全部来源</option><option value="workbench">工作台</option><option value="codex-official">Codex 官方</option><option value="claude-native">Claude 原生</option></select><select value={provider} onChange={(event) => setProvider(event.target.value)}><option value="all">全部主脑</option><option value="codex">Codex</option><option value="claude">Claude</option></select><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">全部状态</option><option value="running">执行中</option><option value="completed">已完成</option><option value="failed">失败</option><option value="archived">已归档</option></select>{section === "health" && <select value={health} onChange={(event) => setHealth(event.target.value)}><option value="all">全部健康状态</option><option value="healthy">正常</option><option value="missing-workspace">工作区失效</option><option value="inconsistent-index">索引不一致</option></select>}</div>
