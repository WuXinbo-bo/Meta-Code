...
export const HELP_TOPICS: HelpTopic[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      { title: "自动与立即备份", body: "自动备份默认每天在工作台空闲时运行，也可以关闭；立即备份随时使用同一条安全流水线。系统只保留最近两个通过验证的恢复点，新恢复点失败时不会删除旧恢复点。" },
      { title: "验证", body: "每份恢复点发布前都会检查文件哈希和数据库完整性，并在隔离目录完成一次装载演练。重新验证不会修改当前个人数据。" },
      { title: "回到备份点", body: "桌面版会先确认没有运行中的任务，再关闭后端、恢复数据并自动重启。恢复失败会自动换回恢复前的数据；工作区源码和可重新下载的 CLI 不在恢复范围内。" }
    ]
  },
  {
    id: "updates",
