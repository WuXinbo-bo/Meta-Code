  ...
  return <section className="content-view settings-view">
    ...
    {settingsSection === "ai" && aiSettingsPage === "providers" && <div id="native-provider-settings" className="settings-section settings-ai-models">
      ...
      <div className="settings-provider-config-grid">
        {claudeProviderControl && <ProviderSettingsPanel control={claudeProviderControl} focused={providerTab === "claude"} subtitle="Anthropic Messages API" onFocus={() => onProviderChange("claude")}><form className="settings-grid" onSubmit={(event) => event.preventDefault()}>
          <label><span>模型 {claudeModels.length ? <em>{claudeModels.length} 个可用</em> : null}</span><input list="claude-model-options" autoComplete="off" value={form.claude.model} onChange={(event) => updateClaude("model", event.target.value)} placeholder="Claude 模型" /><datalist id="claude-model-options">{claudeModels.map((item) => <option key={item.id} value={item.id}>{item.displayName}</option>)}</datalist></label>
          <label className="wide"><span>Base URL</span><input type="url" autoComplete="url" value={form.claude.baseUrl} onChange={(event) => updateClaude("baseUrl", event.target.value)} placeholder="https://api.anthropic.com" /></label>
          <label className="wide"><span>API Key {settings.claude.apiKeyConfigured && <em>已配置</em>}</span><input type="password" autoComplete="new-password" value={form.claude.apiKey} onChange={(event) => updateClaude("apiKey", event.target.value)} placeholder={settings.claude.apiKeyConfigured ? "输入新值以替换" : "sk-ant-..."} /></label>
          {settings.claude.apiKeyConfigured && <label className="check-row wide"><input type="checkbox" checked={form.claude.clearApiKey} onChange={(event) => updateClaude("clearApiKey", event.target.checked)} /><span>清除已保存的 API Key</span></label>}
          <div className="settings-inline-actions wide">{claudeProviderControl.operations.discoverModels && <button type="button" disabled={detectingClaudeModels} onClick={discoverClaudeModels}><Search size={14} />{detectingClaudeModels ? "正在探测" : "探测模型"}</button>}{claudeProviderControl.operations.testConnection && <button type="button" disabled={checkingClaudeRuntime} onClick={() => void testRuntimeApi("claude")}><Plug size={14} />测试连接</button>}</div>
          {claudeModelNotice && <p className={`settings-discovery-note wide ${claudeModelNotice.startsWith("已发现") ? "success" : "error"}`}>{claudeModelNotice}</p>}
        </form></ProviderSettingsPanel>}
        {codexProviderControl && <ProviderSettingsPanel control={codexProviderControl} focused={providerTab === "codex"} subtitle="OpenAI-compatible API" onFocus={() => onProviderChange("codex")}><form className="settings-grid" onSubmit={(event) => event.preventDefault()}>
          <label><span>模型 {codexModels.length ? <em>{codexModels.length} 个可用</em> : null}</span><input list="codex-model-options" autoComplete="off" value={form.model} onChange={(event) => update("model", event.target.value)} placeholder="Codex 模型" /><datalist id="codex-model-options">{codexModels.map((item) => <option key={item.id} value={item.id}>{item.displayName}</option>)}</datalist></label>
          <label className="wide"><span>Base URL</span><input type="url" autoComplete="url" value={form.baseUrl} onChange={(event) => update("baseUrl", event.target.value)} placeholder="留空使用 OpenAI 默认地址" /></label>
          <label className="wide"><span>API Key {settings.apiKeyConfigured && <em>已配置</em>}</span><input type="password" autoComplete="new-password" value={form.apiKey} onChange={(event) => update("apiKey", event.target.value)} placeholder={settings.apiKeyConfigured ? "输入新值以替换" : "sk-..."} /></label>
          {settings.apiKeyConfigured && <label className="check-row wide"><input type="checkbox" checked={form.clearApiKey} onChange={(event) => update("clearApiKey", event.target.checked)} /><span>清除已保存的 API Key</span></label>}
          <div className="settings-inline-actions wide">{codexProviderControl.operations.discoverModels && <button type="button" disabled={detectingCodexModels} onClick={discoverCodexModels}><Search size={14} />{detectingCodexModels ? "正在探测" : "探测模型"}</button>}{codexProviderControl.operations.testConnection && <button type="button" disabled={checkingCodexRuntime} onClick={() => void testRuntimeApi("codex")}><Plug size={14} />测试连接</button>}</div>
          {codexModelNotice && <p className={`settings-discovery-note wide ${codexModelNotice.startsWith("已发现") ? "success" : "error"}`}>{codexModelNotice}</p>}
        </form></ProviderSettingsPanel>}
