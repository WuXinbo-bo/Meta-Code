    ...
    <div className="task-mode-grid">
      {taskTypes.map((item) => {
        ...
        return <button type="button" key={item.engine} className={`task-mode-card ${selected === item.engine ? "selected" : ""}`} disabled={!available || busy} onClick={() => setSelected(item.engine)}>
          ...
          <span><strong>{item.title}</strong><small>{control?.identity.transport === "acp" ? "ACP 标准接入" : "原生增强"} · {item.description}</small><p>{item.detail}</p></span>
    ...
    {settingsSection === "ai" && aiSettingsPage === "providers" && <div id="native-provider-settings" className="settings-section settings-ai-models">
      ...
      <div className="settings-provider-config-grid">
        {claudeProviderControl && <ProviderSettingsPanel control={claudeProviderControl} focused={providerTab === "claude"} subtitle="Anthropic Messages API" onFocus={() => onProviderChange("claude")}><form className="settings-grid" onSubmit={(event) => event.preventDefault()}>
          <label><span>模型 {claudeModels.length ? <em>{claudeModels.length} 个可用</em> : null}</span><input list="claude-model-options" value={form.claude.model} onChange={(event) => updateClaude("model", event.target.value)} placeholder="Claude 模型" /><datalist id="claude-model-options">{claudeModels.map((item) => <option key={item.id} value={item.id}>{item.displayName}</option>)}</datalist></label>
          <label className="wide"><span>Base URL</span><input value={form.claude.baseUrl} onChange={(event) => updateClaude("baseUrl", event.target.value)} placeholder="https://api.anthropic.com" /></label>
          <label className="wide"><span>API Key {settings.claude.apiKeyConfigured && <em>已配置</em>}</span><input type="password" value={form.claude.apiKey} onChange={(event) => updateClaude("apiKey", event.target.value)} placeholder={settings.claude.apiKeyConfigured ? "输入新值以替换" : "sk-ant-..."} /></label>
        ...
        {codexProviderControl && <ProviderSettingsPanel control={codexProviderControl} focused={providerTab === "codex"} subtitle="OpenAI-compatible API" onFocus={() => onProviderChange("codex")}><form className="settings-grid" onSubmit={(event) => event.preventDefault()}>
          <label><span>模型 {codexModels.length ? <em>{codexModels.length} 个可用</em> : null}</span><input list="codex-model-options" value={form.model} onChange={(event) => update("model", event.target.value)} placeholder="Codex 模型" /><datalist id="codex-model-options">{codexModels.map((item) => <option key={item.id} value={item.id}>{item.displayName}</option>)}</datalist></label>
          <label className="wide"><span>Base URL</span><input value={form.baseUrl} onChange={(event) => update("baseUrl", event.target.value)} placeholder="留空使用 OpenAI 默认地址" /></label>
          <label className="wide"><span>API Key {settings.apiKeyConfigured && <em>已配置</em>}</span><input type="password" value={form.apiKey} onChange={(event) => update("apiKey", event.target.value)} placeholder={settings.apiKeyConfigured ? "输入新值以替换" : "sk-..."} /></label>
