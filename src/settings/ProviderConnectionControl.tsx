  ...
  return <form className="agent-market-profiles provider-connection-control" onSubmit={(event) => { event.preventDefault(); void save(); }}>
    ...
    {loading ? <div className="agent-market-control-loading"><LoaderCircle className="spin" size={13} />正在读取 Agent 配置能力</div> : <>
      ...
      <div className="agent-market-profile-form">
        ...
        <label><span>配置来源</span><select value={profileDraft.authMode === "system-profile" ? "system-profile" : "workbench"} onChange={(event) => setProfileDraft((current) => ({ ...current, authMode: event.target.value === "system-profile" ? "system-profile" : "native-account" }))}><option value="workbench">工作台独立</option><option value="system-profile">复用系统账号</option></select></label>
        {configuration?.authMethods.length ? <label className="wide"><span>登录方式</span><select value={profileDraft.authMethodId} onChange={(event) => { const authMethodId = event.target.value; setProfileDraft((current) => ({ ...current, authMethodId, env: { ...current.env, ...Object.fromEntries((configuration.fields || []).filter((field) => field.kind !== "secret" && field.defaultValue !== undefined && (!field.authMethodIds?.length || field.authMethodIds.includes(authMethodId))).map((field) => [field.env, field.defaultValue || ""])) } })); }}>{configuration.authMethods.map((method) => <option key={method.id} value={method.id} disabled={method.type === "terminal"}>{method.name}{method.type === "terminal" ? "（需要终端）" : ""}</option>)}</select>{selectedAuthMethod?.description && <small>{selectedAuthMethod.description}</small>}</label> : null}
        ...
        {!activeFields.length && <><label className="wide"><span>Base URL</span><input value={profileDraft.baseUrl} onChange={(event) => setProfileDraft((current) => ({ ...current, baseUrl: event.target.value }))} placeholder="https://api.example.com" /></label><label><span>API Key 变量</span><input value={profileDraft.apiKeyEnv} onChange={(event) => setProfileDraft((current) => ({ ...current, apiKeyEnv: event.target.value }))} placeholder="API_KEY" /></label><label><span>Base URL 变量</span><input value={profileDraft.baseUrlEnv} onChange={(event) => setProfileDraft((current) => ({ ...current, baseUrlEnv: event.target.value }))} placeholder="BASE_URL" /></label><label className="wide"><span>API Key {profileDraft.apiKeyConfigured ? "· 已保存" : ""}</span><input type="password" value={profileDraft.apiKey} onChange={(event) => setProfileDraft((current) => ({ ...current, apiKey: event.target.value }))} placeholder={profileDraft.apiKeyConfigured ? "留空保持原值" : "输入密钥"} /></label></>}
