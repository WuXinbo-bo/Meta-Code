...
Meta Code 当前产品版本为 `0.1.6`。`package.json` 是唯一版本源；代码、发布清单和设置页不得各自维护版本常量。
...
当前 `0.1.6` 只开放 `check` 能力；`download`、`apply` 和 `launcher` 能力均为关闭状态。后续 Launcher 接入时通过能力协商开放，而不是在 UI 中伪造进度。
